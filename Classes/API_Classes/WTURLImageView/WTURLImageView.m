...
+ (UIImage*) getCachedImageByUrlString : (NSString*) urlString
{
    ...
    return [(EGOCache *)[WTURLImageView sharedImageCache] imageForKey:cacheKey];
}
              ...
              failure:(void (^)(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error))failure
{
    ...
    if (!(options & WTURLImageViewOptionDontUseDiskCache)) {
        ...
        {
            EGOCache *cache = (EGOCache *)[WTURLImageView sharedImageCache];
        ...
        {
            UIImage *cachedImage = [(EGOCache *)[[self class] sharedImageCache] imageForKey:cacheKey];
            if (cachedImage) {
    ...
    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        if ([[urlRequest URL] isEqual:[[self.requestOperation request] URL]]) {
            ...
            if (!(options & WTURLImageViewOptionDontSaveDiskCache)) {
                EGOCache *cache = (EGOCache *)[[self class] sharedImageCache];
...
+ (void) clearAllCache
{
    [(EGOCache *)[[self class] sharedImageCache] clearCache];
}

+ (void) setDiskCacheDefaultTimeOutInterval : (NSTimeInterval) timeout
{
    [(EGOCache *)[[self class] sharedImageCache] setDefaultTimeoutInterval: timeout];
}
