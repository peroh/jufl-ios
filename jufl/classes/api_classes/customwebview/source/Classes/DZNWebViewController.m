...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(dismissWebViewController:)];
...
- (void)presentActivityControllerWithItem:(id)item andTitle:(NSString *)title
{
    ...
    controller.completionWithItemsHandler = ^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError) {
        DLog(@"completed dialog - activity: %@ - finished flag: %d", activityType, completed);
