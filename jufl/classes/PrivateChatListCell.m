...
- (void)setPrivateChatListCellData :(PrivateChatListModel *)chatList {
    ...
    if([chatList.lastChatDateTime isYesterday]) {
        self.timeLbl.text = @"Yesterday";
    }
    else if ([chatList.lastChatDateTime isToday]) {
        self.timeLbl.text = [NSString stringWithFormat:@"%@",[chatList.lastChatDateTime stringWithFormat:@"hh:mm a"]];
    }
    else {
        self.timeLbl.text = [chatList.lastChatDateTime string];
    }
