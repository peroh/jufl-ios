...
- (void)initializeView {
   ...
   // [self performSelector:@selector(getPhotosImage) withObject:self afterDelay:2.0];
...
- (void)getLocations {
   //NSString *strURL=[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=%f,%f&rankby=distance&types=cafe|restaurant|hospital|food|point_of_interest|political|establishment&key=AIzaSyB0EAoRACMQPUoiPheg-nKQi3cJx6Ot1MI",coord.latitude,coord.longitude];
    
    if ([Location  isLocationServiceOn]) {
        shouldFetchLocation = NO;
        NSDictionary *parameters = @{@"location":kGoogleLatLong,@"key":kGoogleApiKey,@"rankby":KDistance,@"type":@"cafe|restaurant|food"};
        
        [LocationModel getNearbyPlacesWithGivenLocationFromGoogleAPIWithParams:parameters completion:^(NSArray *locations, NSError *error) {
...
- (void)getSearchedPlaceDetails:(LocationModel *)selectedLocation {
  //  https://maps.googleapis.com/maps/api/place/details/json?placeid=ChIJ69LRWXQeDTkR4JqoXQgUtVg&key=AIzaSyB0EAoRACMQPUoiPheg-nKQi3cJx6Ot1MI
    
    
    if ([Location  isLocationServiceOn]) {
        NSDictionary *parameters = @{@"key":kGoogleApiKey,@"placeid":selectedLocation.locationPlaceId};
        
        [LocationModel getPlaceDetailFromGoogleAPIWithParams:parameters completion:^(NSArray *locations, NSError *error) {
            [Utils createMainQueue:^{
                
                if (locations.count > 0) {
                    
                     NSDictionary *aDictLocation=[[(NSDictionary*)[locations objectAtIndex:0] objectForKey:@"geometry"] objectForKey:@"location"];
                    DLog(@"SELECTED ADDRESS :%@",aDictLocation);
                    
                    float latitude = [[aDictLocation valueForKey:@"lat"] floatValue];
                    float longitude = [[aDictLocation valueForKey:@"lng"] floatValue];
                    NSString *name = [(NSDictionary*)[locations objectAtIndex:0] valueForKey:@"name"];
                    [self getSearchedPlacesWithPlaceLat:latitude placeLong:longitude placeNmae:name];
...
- (void)getSearchedPlacesWithPlaceLat:(float)latitude placeLong:(float)longitude placeNmae:(NSString*)name {
    
    //NSString *strURL=[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=%f,%f&rankby=distance&name=%@&sensor=false&key=AIzaSyB0EAoRACMQPUoiPheg-nKQi3cJx6Ot1MI",coord.latitude,coord.longitude,name];

    if ([Location  isLocationServiceOn]) {
       // NSString *latLongString = [NSString stringWithFormat:@"%.10f,%.10f",latitude,longitude];
        NSString *latLongString = [NSString stringWithFormat:@"%f,%f",latitude,longitude];
         NSDictionary *parameters = @{@"location":latLongString,@"key":kGoogleApiKey,@"rankby":KDistance,@"name":name};
...
- (void)autoComplete:(NSString *)text {
    [LocationModel autocompleteGooglePlacesWithText:text withCompletion:^(NSArray *locations, NSError *error) {
        [Utils createMainQueue:^{
            if (locations.count) {
...
{
   // https://maps.googleapis.com/maps/api/place/textsearch/json?location=19.017615,72.856164&radius="200"&query="for"&key=AIzaSyB0EAoRACMQPUoiPheg-nKQi3cJx6Ot1MI
    

    NSDictionary *parameters = @{@"location":kGoogleLatLong,@"key":kGoogleApiKey,@"radius":kRadius,@"query":text};
    [LocationModel getSeartchTextPlacesWithGivenLocationFromGoogleAPIWithParams:parameters completion:^(NSArray *locations, NSError *error) {
        [Utils createMainQueue:^{
            self.autocompleteTableView.hidden = YES;
            if (locations.count > 0) {
                self.locationArray = [NSMutableArray arrayWithArray:locations];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([tableView isEqual:self.autocompleteTableView]) {
        ...
        [self getSearchedPlaceDetails:location];
        RESIGN_KEYBOARD
    }
    else {
...
- (void)getPhotosImage {
   ...
   // https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=CmRdAAAAV2pTc3gB0kaB7_udWivZbOSsWUAxrGuOb8C3pTvVNfe_nfy9DYZ80OSNw_PgyshTRVRvCzepKM2FgkcmXI7LyDRZkX4dzfymwmze9gmw8sDso2qYNFRVhSq_jfkkZvrOEhAgd41EDaON2Jnl0HpJDcgUGhQX6IzgrbDjoBIkwqx1N3scxjI-iw&key=AIzaSyB0EAoRACMQPUoiPheg-nKQi3cJx6Ot1M
    
    if ([Location  isLocationServiceOn]) {
        NSDictionary *parameters = @{@"photoreference":@"CmRdAAAAXn0VBy1faq8vVmhRVxJTOgrVXvFCmM1DJLHJh2Xzmwn2mlGUhwoGTR9UaWTwdzT5GTv962FthmM6lA9bLgUNbbraE9HYWjm2AcTRBJ25IvfVp1DAWmANZSsbZFIuheEzEhBCw0axAicbjOeYGpHSOrzjGhS2Mebvaz2lkLhfLQzPjaZ_v3YMKg",@"key":kGoogleApiKey,@"maxwidth":@"400"};
        
        [LocationModel getPlacePhotosFromGoogleAPIWithParams:parameters completion:^(NSArray *locations, NSError *error) {
            [Utils createMainQueue:^{
                if (locations.count > 0) {
